...
// function is const means it can not modify state of the instance
...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    if (logicalSize == allocatedSize){
        // increase the size of elems
        allocatedSize = 2 * allocatedSize;
        DataPoint* temp = new DataPoint[allocatedSize]();
...
DataPoint HeapPQueue::peek() const {
    ...
    if (logicalSize == 0) error("Heap is empty");
    return elems[1];
}
...
DataPoint HeapPQueue::dequeue() {
    ...
    if (logicalSize == 0) error("Can not deque from empty array");

    _Swap(1, logicalSize);
    DataPoint item = elems[logicalSize];
    logicalSize -= 1;
    _BubbleDown(1);
    return item;
}
...
// recursive function
void HeapPQueue::_BubbleUp(int index){
    ...
    if (elems[index].weight < elems[parent_index].weight) _Swap(index, parent_index);
    _BubbleUp(parent_index);
}


void HeapPQueue::_BubbleDown(int parent_index){

    // pick the smallest child
    int left_child = int(2*parent_index);
    int right_child = left_child + 1;
    int correct_index = -1;

    // check if right child within bounds, this implies that both children exists
    if (right_child <= logicalSize){

        // if left child is smaller
        if (elems[left_child].weight < elems[right_child].weight){

            if (elems[parent_index].weight > elems[left_child].weight) correct_index = left_child;

        }else{
            // right child is smaller
            if (elems[right_child].weight < elems[parent_index].weight) correct_index = right_child;
        }

    }else if (left_child <= logicalSize){
        // only left child exist
        if (elems[parent_index].weight > elems[left_child].weight) correct_index = left_child;
    }

    if (correct_index != -1){
        _Swap(parent_index, correct_index);
        _BubbleDown(correct_index);
    }
...
void HeapPQueue::_Swap(int x, int y){
    // NOTE: c++ compiler does not free the memory by default when the function exists, therefore we have to do that here

    // this variable will be allocated on stack which will be released on its on after finction exists
    DataPoint tmp = elems[x];
    elems[x] = elems[y];
    elems[y] = tmp;
...
void HeapPQueue::printDebugInfo() {
    ...
    cout<<logicalSize<<","<<allocatedSize<<"\n";
    for (int i=1; i<=logicalSize; i++) cout<< elems[i].weight;
}
