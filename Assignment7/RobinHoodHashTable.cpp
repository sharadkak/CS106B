...
RobinHoodHashTable::RobinHoodHashTable(HashFunction<string> hashFn) {
    ...
    hash_func = hashFn;
    logicalSize = 0;
    allocatedSize = hash_func.numSlots();

    elems = new Slot[allocatedSize];

    for (int i=0; i<allocatedSize;i++){
        // initally all lots have distance set to constant negative value
...
int RobinHoodHashTable::_findIndex(const std::string &elem) const{
    // to find the item we have to check the distances

    int hash_index = hash_func(elem);
    int local_distance = 0;
    while((elems[hash_index].value != elem) && (elems[hash_index].distance >= local_distance)){
        if (elems[hash_index].distance == EMPTY_SLOT) break;
        local_distance += 1;
        hash_index = (hash_index+1)%allocatedSize;
    }

    if((elems[hash_index].value == elem) && (elems[hash_index].distance != EMPTY_SLOT)) return hash_index;
    return -1;
}
...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    if (logicalSize==allocatedSize) return false;
    // if the element is already present
    if (_findIndex(elem) != -1) return false;

    // insert
    int hash_index = hash_func(elem);

    // else, find the right slot as per distance and then insert
    int local_distance=0;
    while(true){

        // if the index is already empty
        if (elems[hash_index].distance == EMPTY_SLOT){
            // we insert the current elem
            elems[hash_index].value = elem;
            elems[hash_index].distance = local_distance;
            logicalSize += 1;
            return true;
        }
        else if ((elems[hash_index].distance != EMPTY_SLOT) && (local_distance > elems[hash_index].distance)){
            // insert
            Slot prev_elem;
            prev_elem.value = elems[hash_index].value;
            prev_elem.distance = elems[hash_index].distance+1;
            ...
            hash_index = (hash_index+1)%allocatedSize;
            // displace
            while (elems[hash_index].distance != EMPTY_SLOT){

                // if the distance is larger than current element here and take out inserted element (swap)
                if (prev_elem.distance > elems[hash_index].distance){
                    Slot tmp;
                    tmp.value = elems[hash_index].value;
                    tmp.distance = elems[hash_index].distance;

                    elems[hash_index].value = prev_elem.value;
                    elems[hash_index].distance = prev_elem.distance;

                    prev_elem.value = tmp.value;
                    prev_elem.distance = tmp.distance+1;
                }
                else prev_elem.distance += 1;
                hash_index = (hash_index+1)%allocatedSize;
            }

            // insert the current element at the free spot
            elems[hash_index].value = prev_elem.value;
            elems[hash_index].distance = prev_elem.distance;
            return true;
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    if (logicalSize==0) return false;

    if (_findIndex(elem) != -1) return true;
    return false;
}
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    if (index != -1){
        ...
        // now displace elements
        while (elems[index].distance > 0){

            elems[previous].value = elems[index].value;
            elems[previous].distance = elems[index].distance-1;
            previous = index;
            index = (index+1)%allocatedSize;
        }

        // remove the previous element
        elems[previous].distance = EMPTY_SLOT;
        return true;
    }
    return false;
}
...
void RobinHoodHashTable::printDebugInfo() const {
    ...
    cout<<"logical"<<logicalSize<<"\n";
    for (int i=0; i<allocatedSize;i++) {
        cout<<elems[i].value<<","<<elems[i].distance<<","<<hash_func(elems[i].value)<<"\n";
    }
}
