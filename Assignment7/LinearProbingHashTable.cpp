...
LinearProbingHashTable::LinearProbingHashTable(HashFunction<string> hashFn) {
    ...
    // allocate the memory for hash table with slot
    hash_func = hashFn;
...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    // if elem is already in hash table then dont insert it
    if (_findIndex(elem) != -1) return false;
    if (logicalSize == allocatedSize) return false;

    int hash_index = hash_func(elem);
    int intial_index = hash_index;

    // go to the hash index and if there is space then insert the elem and return true
    if ((elems[hash_index].type == SlotType::EMPTY) || (elems[hash_index].type == SlotType::TOMBSTONE)){
        logicalSize += 1;
        elems[hash_index].value = elem;
        elems[hash_index].type = SlotType::FILLED;
        return true;

    }else{

        hash_index = (hash_index+1) % allocatedSize;
        // iterate over the next slots and insert in the free slot
        while (hash_index != intial_index){
            if ((elems[hash_index].type == SlotType::EMPTY) || (elems[hash_index].type == SlotType::TOMBSTONE)){
                logicalSize += 1;
                elems[hash_index].value = elem;
                elems[hash_index].type = SlotType::FILLED;
...
int LinearProbingHashTable::_findIndex(const std::string &elem) const{
    ...
    while ((elems[hash_index].value != elem) && (elems[hash_index].type != SlotType::EMPTY)){
        hash_index = (hash_index+ 1) % allocatedSize;
        elements_seen+=1;

        if (elements_seen == allocatedSize) break;
    }

    if ((elems[hash_index].type == SlotType::FILLED) && (elems[hash_index].value == elem)) return hash_index;
...
bool LinearProbingHashTable::contains(const string& elem) const {
    /* TODO: Delete this comment and the next lines, then implement this function. */
    if (_findIndex(elem) >= 0) return true;
    return false;
}
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    // find the index for the elem in hash table
    int index = _findIndex(elem);

    if (index == -1) return false;
    else{
        // remove the element by setting slot type to tombstone
...
void LinearProbingHashTable::printDebugInfo() const {
    ...
    for (int i=0; i < allocatedSize; i++){
        cout<<elems[i].value<<","<<elems[i].type<<"\n";
    }
}
