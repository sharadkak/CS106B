...
Nucleotide* toStrand(const string& str) {
    ...
    if (str == "") return nullptr;
        ...
        for (auto c : str){
            ...
            if (previous != nullptr) previous->next = tmp;
            tmp->prev = previous;
            previous = tmp;
        }
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    Nucleotide* target_head = target;

    while (dna != nullptr){

        // go through the dna and find the first letter of target and then look for whole target from there

        if (target->value == dna->value)
        {
            Nucleotide* ptr = dna;
            // check if the whole sequence matches
            while ((target != nullptr) && (dna != nullptr)){
            ...
            if (target == nullptr) return ptr;

            if (dna == nullptr) return nullptr;
            else {
                // if unsucessful, set dna to the next to where we found the match
                dna = ptr->next;
            }
        }
        else dna = dna->next;
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    Nucleotide* node = findFirst(dna, target);
    if (node == nullptr) return false;

    Nucleotide* trash = nullptr;
    Nucleotide* previous = nullptr;
    // if the node is head node, modify dna
    if (node == dna) {
        ...
        // delete last node
        previous->next = node;
        if (node != nullptr) node->prev = previous;
    }
    return true;
}
