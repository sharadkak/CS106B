...
bool helperDisaster(const Map<string, Set<string>>& roadNetwork, Vector<string>& cities, int numCities, Set<string>& supplyLocations, Map<string, bool>& coverageMap);

bool checkAll(Map<string, bool> collection){
    for (string key: collection){
        if (collection[key] == false) return false;
    }
    return true;
}
...
bool helperDisaster(const Map<string, Set<string>>& roadNetwork, Vector<string>& cities, int numCities, Set<string>& supplyLocations, Map<string, bool>& coverageMap){
    ...
    // base case
    // if no more cities are left to explore
    if (cities.size() == 0){
        if (checkAll(coverageMap)) return true;
        return false;
    }
    ...
    for(int i=0; i<cities.size();i++){
        ...
        Set<string> city_neighbours = roadNetwork[city];

        // if the city does not have coverage
        if (coverageMap[city] == false){

            // check if we can already reach the city through its neighbours
            if (supplyLocations.size()>0){
                // for each location here check if its the neighbour of city
                for(string location : supplyLocations){
                    if (city_neighbours.contains(location)){
                        coverageMap[city] = true;
                        return true;
                    }
                }
            }

            // since it cant be given coverage we have to provide it coverage

            // 1. stockpiling at this city itself
            if (supplyLocations.size() < numCities){
                supplyLocations.add(city);
                coverageMap[city] = true;
                cities.remove(i);
                bool return_value = helperDisaster(roadNetwork, cities, numCities - 1, supplyLocations, coverageMap);
                if (return_value){
                    ...
                    // coverageMap.put(city, false);

                    // 2. iterate over all neighbours and stockpile one of them at a time so that we can reach this city
                    for (string neighbour : city_neighbours){
                        supplyLocations.add(neighbour);
                        coverageMap[neighbour] = true;
                        bool return_value = helperDisaster(roadNetwork, cities, numCities - 1, supplyLocations, coverageMap);
                        if (return_value) return true;

                        //unchoose
                        supplyLocations.remove(neighbour);
                        coverageMap[neighbour] = false;

                    }
                    coverageMap[city] = false;
                    cities.insert(i, city);
                }
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    if (numCities < 0) error("numCities must be positive");

    // make a map of all cities not covered
    Map<string, bool> coverageMap;
    for (string name: roadNetwork.keys()) coverageMap.put(name, false);

    Vector<string> cities = roadNetwork.keys();
    return helperDisaster(roadNetwork, cities, numCities, supplyLocations, coverageMap);
}
...
PROVIDED_TEST("Works for map with one city, and produces output.") {
    ...
    //Don't check locations0; since the function returned false, the values there can be anything.
