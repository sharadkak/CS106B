...
// in this recursion we make a small choice of choosing a shift or not
// this is a permutation problem as order of the shift matters
void helperFunction(Vector<Shift> &shifts_vector, int maxHours, Set<Shift> &chosen, int total_value, Vector<Set<Shift>> &results){
    // base case
    if (maxHours == 0) {
        // we explore one shift plan add it to the results vector
        results.add(chosen);
        return;
    }

    // traverse the set of shifts
    for (int i=0; i<shifts_vector.size(); i++){

        Shift shift = shifts_vector[i];

        // first check if this shift could be assigned
        // it should not exceed maxHours
        int shift_time = shift.endHour - shift.startHour;
        if (shift_time <= maxHours){

            // should not overlap with already assigned shifts
            for (Shift each: chosen){
                // check for overlap
                if ((shift.startHour >= each.endHour) || (shift.endHour <= each.startHour)) continue;
                else break;
            }

            // if it can be assigned then you can choose it or not choose it
            // make two recursive calls here

            // choose and recurse
            helperFunction(shifts_vector, maxHours, chosen, total_value, results);
            chosen.add(shift);
            shifts_vector.remove(i);
            helperFunction(shifts_vector, maxHours - shift_time, chosen, total_value + shift.value, results);

            // unchoose
            chosen.remove(shift);
            shifts_vector.insert(i, shift);
        }
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    if (maxHours < 0) error("maxHours can not be negative");

    Vector<Shift> shifts_vector;
    // copy all the shifts in a vector
    for (Shift shift: shifts){
        shifts_vector.add(shift);
    }

    Set<Shift> chosen;
    Vector<Set<Shift>> results;
    helperFunction(shifts_vector, maxHours, chosen, 0, results);

    // find the best one in results vector and return it
    Set<Shift> best_shift;
    int best_score = -1;
    for (auto each : results){
        // each is a set
        int curr_value = 0;
        for (auto shift: each) {
            curr_value += shift.value;
        }

        if (curr_value > best_score) best_shift = each;
    }
