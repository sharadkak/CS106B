...
void helperEmphasesOf(Vector<string>& tokens, Set<string>& result, Vector<string>& intermediate);

void helperEmphasesOf(Vector<string>& tokens, Set<string>& result, Vector<string>& intermediate){

    // base case
    if (tokens.size()==0){
        // form a string using tokens from intermediate and move it to result set
        string final_string;
    ...
    // check if a token is a word by checking its first letter
    if (isalpha(token[0])){

        // choose
        string tmp = toUpperCase(token);

        // make two recursive calls (basically once we choose and once we don't)
        intermediate.add(tmp);
        helperEmphasesOf(tokens, result, intermediate);

        int size = intermediate.size();
        intermediate.remove(size-1);

        tmp = toLowerCase(token);
        intermediate.add(tmp);
        helperEmphasesOf(tokens, result, intermediate);
        ...
        // else we add the token and make the recursive call
...
Set<string> allEmphasesOf(const string& sentence) {
    ...
    Set<string> results;
    Vector<string> inter_string;
    helperEmphasesOf(tokens, results, inter_string);
    return results;
}
