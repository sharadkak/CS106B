...
double weightOnBackOf(int row, int col, int pyramidHeight) {
    ...
    // inbound check
    if ((row > pyramidHeight) || (col>row)) error("row and col are not within bounds");
    ...
    // recursive case
    if (col == 0){
        // means it is either leftmost or rightmost child and only has one predecessor,
        // hence make only one recusive call
        return 80 + weightOnBackOf(row-1, col, pyramidHeight)/2;
    }else if (row==col) return 80 + weightOnBackOf(row-1, col-1, pyramidHeight)/2;
    else{
        // the child is somewhere in between, hence two recursive calls
        return 80 + 80 + (weightOnBackOf(row-1, col-1, pyramidHeight)/2) + (weightOnBackOf(row-1, col, pyramidHeight)/2);
    }
}
