...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
     ...
     * this function.
     */
    if (kGramLength <= 0) error("kGramLength must be greater than zero");

    Map<string, double> k_grams;

    if (str.length() < kGramLength) return k_grams;

    string trigram;

    for (int i=0; i < str.length() - (kGramLength-1); i++){
        trigram = str.substr(i, kGramLength);

        if (k_grams.containsKey(trigram)) k_grams[trigram] += 1.0;
        else k_grams[trigram] = 1.0;
    }
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    if (input.size() == 0) error("Error raised");

    double summation = 0.0;
    for (string each : input){

        summation += pow(input[each], 2);
    }

    // if summation is 0, it means all values in the map are zero, hence raise error
    if (summation == 0) error("There must be at least one non zero value");

    double root_value = sqrt(summation);

    Map<string, double> normalized_profile;
    for (string key : input){
        normalized_profile.put(key, input[key]/root_value);
    }
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    if (numToKeep < 0) error("this number should be positive");

    // create a priority queue and load all elements into it
    PriorityQueue<string> queue;
    for (string key: source){
        queue.enqueue(key, source[key]);
    }

    // now dequeue extras with low priority
    for (int i=0; i<(source.size() - numToKeep); i++){
        string removed = queue.dequeue();
    }

    // load the rest of the elements from queue into a new map container
    Map<string, double> result_map;

    for (int i=0; i < numToKeep; i++){
        string removed = queue.dequeue();
        result_map.put(removed, source.get(removed));
    }
    return result_map;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    if (corpora.size() == 0) error("There is nothing in the corpura");

    PriorityQueue<string> results;
    for (Corpus a_corpus: corpora){
        double sim_value = cosineSimilarityOf(textProfile, a_corpus.profile);
        results.enqueue(a_corpus.name, sim_value);
    }

    string removed;
    for (int i=0; i < corpora.size(); i++){
        removed = results.dequeue();
    }
    return removed;
}
