...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Grid<bool> result_grid(terrain.numRows(), terrain.numCols(), false);

    Queue<GridLocation> bfs_queue;
    bool flag = false;
    // first collect points at the source locations with height below water level
    for (GridLocation each : sources){
        double source_level = terrain[each.row][each.col];
        GridLocation location = {each.row, each.col};
        if (source_level <= height){
            flag = true;    // set the flag just once to track if there is at least one point flooded
            result_grid[location.row][location.col] = true;
            bfs_queue.enqueue(location);
        }
    }

    if (!flag) return result_grid;

    Map<string, Vector<int>> neigh_cordinates = {{ "up", {-1,0}}, { "down", {1,0} },
                                                      {"left", {0,-1}}, {"right", {0,1}}};
    while (!bfs_queue.isEmpty()){
        //
        GridLocation location = bfs_queue.dequeue();
        // visit its neighbours
        for (string neigh_type : neigh_cordinates){
            int row_index = location.row + neigh_cordinates[neigh_type][0];
            int col_index = location.col + neigh_cordinates[neigh_type][1];

            // proceed further only if the indices are in bounds
            if (terrain.inBounds(row_index, col_index)){
                double neigh_value = terrain[row_index][col_index];

                if ((result_grid[row_index][col_index] == false) && (neigh_value <= height)) {
                    result_grid[row_index][col_index] = true;
                    GridLocation new_location = {row_index, col_index};
                    bfs_queue.enqueue(new_location);
