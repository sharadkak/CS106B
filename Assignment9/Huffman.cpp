...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    if (str.length() < 2) error("Length must be greater than 2");
    ...
    for (int i=0; i<str.length(); i++){
        ...
        if (mapping.containsKey(str[i])) mapping[str[i]] += 1;
        else mapping[str[i]] = 1;
    }

    if (mapping.size() < 2) error("String must be greater than 2");

    // create a priority queue with
    PriorityQueue<EncodingTreeNode*> pq;
    // Initialize a priority queue with singleton trees
    for (char key: mapping){
        EncodingTreeNode* node = new EncodingTreeNode();
        node->ch = key;
        pq.enqueue(node, mapping[key]);
    }


    while (pq.size() > 1){
        // withdraw two trees and merge them
        ...
        if (pq.size() != 0) pq.enqueue(temp, priority);
        else head = temp;
    }

    if (!pq.isEmpty() && head!= nullptr){
...
string decodeText(Queue<Bit>& bits, EncodingTreeNode* tree) {
    ...
    while (!bits.isEmpty()){
        ...
        Bit value = bits.dequeue();

        if (value == 0) curr = curr->zero;
        else if (value == 1) curr = curr->one;
...
bool findBitString(char letter, EncodingTreeNode* tree, Vector<Bit>& temp){
    ...
    temp.add(0);
    if (findBitString(letter, tree->zero, temp)) return true;
    else{
    ...
    temp.add(1);
    if (findBitString(letter, tree->one, temp)) return true;
    else {
        temp.remove(temp.size()-1);
    }
...
Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {
    ...
    for (char each : str){
        Vector<Bit> temp;
        bool bool_value = findBitString(each, tree, temp);
...
void createTree(EncodingTreeNode* tree, Queue<Bit>& bits, Queue<char>& leaves, Bit child_type){
    ...
    // base case
    if (bit == 0) {
        char value = leaves.dequeue();
        node->ch = value;

        if (child_type == 0) tree->zero = node;
        else tree->one = node;
    ...
    if (child_type == 0) {
        tree->zero = node;
        createTree(tree->zero, bits, leaves, 0);
        createTree(tree->zero, bits, leaves, 1);
    }
    else {
        tree->one = node;
        createTree(tree->one, bits, leaves, 0);
        createTree(tree->one, bits, leaves, 1);
    }
...
EncodingTreeNode* decodeTree(Queue<Bit>& bits, Queue<char>& leaves) {
    ...
    EncodingTreeNode* head = new EncodingTreeNode();

    Bit bit = bits.dequeue();
    if (bit == 1){
            head->ch = '*';
    }else if (bit==0) return head;

    createTree(head, bits, leaves,0);
    createTree(head, bits, leaves,1);
...
HuffmanResult compress(const string& text) {
    ...
    Queue<Bit> bits;
    Queue<char> leaves;
    // encode the text using this tree
    Queue<Bit> message = encodeText(text, tree);
    // encode the tree itself into a bits and leaves
    encodeTree(tree, bits, leaves);

    deleteTree(tree);

    HuffmanResult result = {bits, leaves, message};
    return result;
}
